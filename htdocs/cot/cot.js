...
function run_with_jquery(callback) {
	if (typeof jQuery == 'undefined') {
		...
		getScript('http://www.commentonthis.net/cot/jquery.min.js', function() {
			if (typeof jQuery=='undefined') {
...
function cot_hash_node(node) {
	crc = crc32(node.text());
	if(crc < 0) crc = Math.pow(2, 32) + crc;
	return "crc-"+crc.toString(16);
}
...
function cot_init(account_id, filter) {
	...
	run_with_jquery(function() {
		jQuery(filter).each(function(idx, item_el_raw) {
			var item_el = jQuery(item_el_raw);
			...
			cbox_toggle_icon.attr("src", cot_base+"cot/cot.png");
			cbox_toggle_icon.attr("class", "cot_toggle_icon");

			var cbox = jQuery("<div />");
			cbox.attr("class", "cot_linkbox");
			cbox.append(cbox_link_add);
			cbox.append(jQuery("<br />"));
			cbox.append(cbox_link_view);

			var cbox_toggle = jQuery("<div />");
			cbox_toggle.attr("class", "cot_toggle");
			cbox_toggle.append(cbox_toggle_icon);
			cbox_toggle.append(cbox);

			item_el_raw.insertBefore(cbox_toggle.get(0), item_el_raw.firstChild);
		});
	});
}
